...
class ABSADatesetReader:
    ...
    def __read_data__(fname, tokenizer):
        ...
        for i in range(0, len(lines), 5):
            ...
            aspects = lines[i+3].split('_T_')
            polarities = lines[i+4].split('_P_')
            aspects_polarities = []
            for j in range(len(aspects)):
                start = len(text.split(aspects[j])[0].split())
                end = start + len(aspects[j].split()) + 1
                aspects_polarities.append((start,end,int(polarities[j])))
            ...
            data = {
                ...
                'polarity': polarity,
                'aspect_polarity': aspects_polarities,
