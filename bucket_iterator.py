...
class BucketIterator(object):
    ...
    def pad_data(self, batch_data):
        ...
        batch_left_indices = []
        batch_polarity = []
        batch_aspect_polarity = []
        batch_dependency_graph = []
        max_len = max([len(t[self.sort_key]) for t in batch_data])
        aspects_max_len = max(len(t['aspect_polarity']) for t in batch_data)
        for item in batch_data:
            text_indices, context_indices, aspect_indices, left_indices, polarity, aspect_polarity , dependency_graph = \
                item['text_indices'], item['context_indices'], item['aspect_indices'], item['left_indices'],\
                item['polarity'], item['aspect_polarity'] , item['dependency_graph']
            ...
            aspect_polarity.extend([(-1,-1,-1)]*(aspects_max_len-len(aspect_polarity)))
            batch_aspect_polarity.append(aspect_polarity)
        ...
        return { \
                ...
                'aspect_polarity': torch.tensor(batch_aspect_polarity),\
