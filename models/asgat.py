...
class ASGAT3(nn.Module):
    ...
    def forward(self, inputs):
        text_indices, aspect_indices, left_indices, aspect_polarity, adj = inputs
        text_len = torch.sum(text_indices != 0, dim=-1)
        aspect_len = torch.sum(aspect_indices != 0, dim=-1)
        left_len = torch.sum(left_indices != 0, dim=-1)
        aspect_double_idx = torch.cat([left_len.unsqueeze(1), (left_len+aspect_len-1).unsqueeze(1)], dim=1)
        aspect_polarity = aspect_polarity.cpu().numpy().tolist()
        text = self.embed(text_indices)
        text = self.text_embed_dropout(text)
        text_out, (_, _) = self.text_lstm(text, text_len)
        x = F.elu(self.ga(text_out, adj))
        x = self.mask(x, aspect_double_idx)
        current_aspect = x.mean(dim=1).squeeze()
        # current_aspect = F.max_pool2d(x,(x.shape[1],1)).squeeze()
        batches = x.shape[0]
        loss_neu = torch.tensor(0.).to(self.opt.device)
        for i in range(batches):
            aspects = []
            polarity = []
            for a in aspect_polarity[i]:
                if a[0]>=0:
                    aspects.append(x[i][a[0]:a[1]].mean(dim=0).squeeze())
                    polarity.append(a[2])
            loss_neu + self.__get_loss__(aspects,polarity)
        output = self.fc(current_aspect)
        return output,loss_neu/batches
